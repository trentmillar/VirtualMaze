				   ...
				   LPSTR lpCmdLine, int iCmdShow)
{
	...
	Game::ParseCommandLine(lpCmdLine);

	int p = Game::GetParameterIndex("-width");
	if(p)
	{
		screenWidth =atoi(Game::Commands[p]);		 
	}
	Game::SetScreenWidth( screenWidth );

	p = Game::GetParameterIndex("-height");
	if(p)
	{
		screenHeight = atoi(Game::Commands[p]);
	}
	Game::ScreenHeight = screenHeight;

	if(Game::GetParameterIndex("-fullscreen"))
	{
		fullscreen = Game::IsFullScreen = TRUE;
	}
	else{
		fullscreen = Game::IsFullScreen = FALSE;
	}
	...
	/*WNDCLASS cls = {
		CS_DBLCLKS, (WNDPROC)WinProcess, 0, 0, hInstance,
		LoadIcon(NULL, IDI_APPLICATION), LoadCursor(NULL, IDC_ARROW),
		(HBRUSH)GetStockObject(BLACK_BRUSH), NULL, "NETMAZE"};
	...
	Game::mode->init_draw(Game::GetScreenWidth(), Game::ScreenHeight);//&drawinfo);
	//Game::load_graphics();
